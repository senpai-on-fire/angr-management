...
from .apis import CauseBase, DataItemCause, InstrOpcodeCause, InstrOperandCause
...
class RootCauseAnalysis(Analysis):
    def __init__(
        ...
        expression_source: dict[claripy.ast.BV, tuple[int, int]] | None = None,
        config_vars: dict[claripy.ast.BV, tuple[str, int, str, int]] | None = None,
    ):
        self.arch = arch
    ...
    def _expr_root_cause(self, expr: claripy.ast.BV) -> tuple[str, tuple[str, int, str, int] | tuple[int, int]] | None:
        """
